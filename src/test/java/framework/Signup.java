...
public class Signup extends Base {
	...
	WebElement confirmPasswodLabel;
	@FindBy(xpath = "//div[.=' Password must match ']")
	...
	public void selectGender(String value) throws InterruptedException {
		Thread.sleep(3000);
		waitAndClick(gender);
		List<WebElement> opt = driver.findElements(By.xpath("//span[@class='mat-option-text']"));
		int count = opt.size();
		if(!opt.isEmpty()) {
			for (int i=0;i<count;i++) {
	...
	public String getPasswodPlaceholder() {
		return password.getAttribute("placeholder");
	}
	public String getConfirmPassPlaceholder() {
		return confirmPasswod.getAttribute("placeholder");
	}
